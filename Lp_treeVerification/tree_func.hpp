...
tuple<vector<vector<Leaf>>, double> find_k_partite_clique(const vector<double>& x, vector<vector<Leaf>>all_tree_reachable_leaves, int max_clique, double eps, int label, int neg_label, int num_classes, bool dp, int feature_start, bool one_attr, int only_attr,int order){
  ...
  for (int start_tree=0; start_tree < all_tree_reachable_leaves.size(); start_tree = start_tree+max_clique){ 
    ...
    for (int i=0; i<LL_old->size(); i++){ 
      ...
      interval_map<int,Interval>& box = get<0>((*LL_old)[i]);

      if (point_box_dist(x, box, order, feature_start, one_attr, only_attr) > eps)
      {
        // cout << "filter out a box " << point_box_dist(x, box, order, feature_start, one_attr, only_attr) << endl;
        continue;
      }
...
vector<vector<Leaf>> find_reachable_leaves (const vector<double>& x, vector<vector<Leaf>> all_tree_leaves, double eps, int label, int neg_label, int num_classes, int feature_start, bool one_attr, int only_attr, int order){
  ...
  for (int i=0; i< all_tree_leaves.size(); i++){
    if (num_classes <= 2 || neg_label < 0 || ((i % num_classes) == label) || ((i % num_classes) == neg_label)){
      ...
      for (int j=0; j<all_tree_leaves[i].size(); j++){
        ...
        */
        if ((all_tree_leaves[i][j].box.find(-100) == all_tree_leaves[i][j].box.end()) && point_box_dist(x, all_tree_leaves[i][j].box, order, feature_start, one_attr, only_attr)<=eps) {
...
vector<double> find_multi_level_best_score (const vector<double>& x, int label, int neg_label, vector<vector<Leaf>> all_tree_leaves, int num_classes, int max_level, double eps, int max_clique, int feature_start, bool one_attr, int only_attr, bool must_use_dp, int order){
  //pick the reachable leaves on each tree
  vector<vector<Leaf>> all_tree_reachable_leaves = find_reachable_leaves(x, all_tree_leaves, eps, label, neg_label, num_classes, feature_start, one_attr, only_attr, order);  
  //shuffle trees
  ...
  for (int l=0; l<max_level; l++){
    ...
    if (num_classes > 2 && l == 0)
      res = find_k_partite_clique(x, new_nodes_array, max_clique, eps, label, neg_label, num_classes, use_dp, feature_start, one_attr, only_attr,order);
    else
      res = find_k_partite_clique(x, new_nodes_array, max_clique, eps, label, -1, num_classes, use_dp, feature_start, one_attr, only_attr, order);
