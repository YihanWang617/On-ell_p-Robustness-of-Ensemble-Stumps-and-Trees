...
int main(int argc, char** argv){
  ...
  for (int n=start_idx; n<num_attack+start_idx; n++){ //loop all points
    ...
    for (int search_step=0; search_step<max_search; search_step++){
      ...
      if (num_classes <= 2){ 
        cout << "\n^^^^^^^^^^^^^^^^ binary model  ^^^^^^^^^^^^^^^\n";
        vector<double> sum_best = find_multi_level_best_score(ori_X[n], ori_y[n], -1, all_tree_leaves, num_classes, max_level, eps, max_clique, feature_start, one_attr, only_attr, dp, order = order); 
        ...
        for (int neg_label=0; neg_label<num_classes; neg_label++){
          if (neg_label != ori_y[n]){
            ...
            vector<double> sum_best = find_multi_level_best_score(ori_X[n], ori_y[n], neg_label, all_tree_leaves, num_classes, max_level, eps, max_clique, feature_start, one_attr, only_attr, dp, order = order);
