...
from robust_boosting import exp_loss_robust, dtype, fit_plain_stumps, fit_robust_bound_stumps, fit_robust_bound_stumps_tree_Lp, point_box_dist
...
class LeafNode:
    def __init__(self, box, value, y_min_value_map):
        self.box = box
        self.value = value
        self.y_min_value_map = y_min_value_map
...
class Tree:
    def __init__(self, id_=-1, left=None, right=None, w_l=0.0, w_r=0.0, b=0.0, coord=0, loss=0.0):
        ...
        # for leaf node, the node records all the samples that can fall into the node.
        self.X = None
        self.budget = None
    ...
    def to_json(self, node_id):
        obj = {"nodeid": node_id, "depth": 0, "split": self.coord, "split_condition": float(self.b), "yes": (node_id + 1) * 2 - 1, "no": (node_id + 1) * 2, "missing": (node_id + 1) * 2, "children":[]}

        if self.left == None:
            obj["children"].append({"nodeid": (node_id + 1) * 2 - 1, "leaf": float(self.w_l)})#, {"nodeid": (node_id + 1) * 2, "leaf": float(self.w_l + self.w_r)}]
        else:
            obj["children"].append(self.left.to_json((node_id + 1) * 2 - 1))
        # obj = {"nodeid": node_id, "depth": 0, "split": self.coord, "split_condition": float(self.b), "yes": (node_id + 1) * 2 - 1, "no": (node_id + 1) * 2, "missing": (node_id + 1) * 2, "children":[]}
        if self.right == None:
            obj["children"].append({"nodeid": (node_id + 1) * 2, "leaf": float(self.w_l + self.w_r)})
        else:
            obj["children"].append(self.right.to_json((node_id + 1) * 2))
...
class TreeEnsemble:
    def __init__(self, weak_learner, n_trials_coord, lr, min_samples_split, min_samples_leaf, idx_clsf, max_depth, schedule_len,
    ...
    def load_tree(obj):
        if "split" not in obj:
            ...
            left = load_tree(obj["children"][0])
            right = load_tree(obj["children"][1])

            if left == None and right == None:
                root = Tree(obj["nodeid"], None, None, obj["children"][0]["leaf"], obj["children"][1]["leaf"], obj["split_condition"], obj["split"])
            else:
                root = Tree(obj["nodeid"], left, right, 0, 0, obj["split_condition"], obj["split"])


    def load_json(self, filename, X):
        ...
        for idx, x in enumerate(X):
            self.certify_treewise()
            
    # def traverse_tree(self, tree, x):
    ...
    def add_weak_learner(self, tree, apply_lr=True, pre_leaf = True):
        ...
        if pre_leaf:
            self.pre_leaf_node_list = self.leaf_node_list
            self.leaf_node_list = []

        # for node in self.pre_leaf_node_list:
        #     print(node.y_min_value_map)
    ...
    def fit_tree_Lp(self, X_index, X, y, eps, depth, order = 1, budget = None, box = Box(), coord_to_ignore = ()):

        assert order > 0, 'order should larger than 0'
        assert budget is not None, "budget should be given"

        print(coord_to_ignore)

        # if depth == 1:
        #     self.pre_leaf_node_list = self.leaf_node_list
        #     self.leaf_node_list = []
        
        pre_leaf_nodes = self.pre_leaf_node_list
        ...
        # if len(self.trees) == 0:
        #     w_l, w_r, b, coord, loss = self.fit_stumps_over_coords(X, y, np.ones(y.shape[0]), 'robust_bound', eps, depth)
        # else:

        # TODO: here we should calculate gamma.
        # gamma = np.zeros_like(y) + np.inf
        # for node in self.pre_leaf_node_list:
        #     # if node.box.is_intersect(box):
        #     for i, (index, X_i, y_i) in enumerate(zip(X_index, X, y)):
        #         # if point_box_dist(X_i, node.box, order) < eps:
        #         if index in node.y_min_value_map:
        #             gamma[i] = min(gamma[i], node.y_min_value_map[index])

        # gamma = np.exp(-gamma)

        # in_node = (budget == 0)
        if depth < self.schedule_len:
            cur_eps = eps/self.schedule_len * depth
        else:
            cur_eps = eps
        w_l, w_r, b, coord, loss = self.fit_stumps_over_coords_Lp(X_index, X, y, budget, pre_leaf_nodes, cur_eps, eps, order, depth, box, coord_to_ignore=coord_to_ignore)
        ...
        next_coord_to_ignore = (*coord_to_ignore, coord)
        self.max_tree_node_id += 1  # increment the counter


        idx_left = (X[:, tree.coord] < tree.b) | (np.power(np.abs(X[:, tree.coord] - tree.b), order) + budget < eps ** order)
        idx_right = (X[:, tree.coord] >= tree.b) | (np.power(np.abs(X[:, tree.coord] - tree.b), order) + budget < eps ** order)

        print(np.sum(idx_left) - np.sum(X[:, tree.coord] < tree.b))

        budget_left = budget[idx_left]
        budget_right = budget[idx_right]

        box_left = box.get_intersection(Box({tree.coord: (np.float64(-np.inf), np.float64(tree.b))}))
        box_right = box.get_intersection(Box({tree.coord: (np.float64(tree.b), np.float64(np.inf))}))

        idx_left_increase = (X[idx_left, tree.coord] >= tree.b) & (np.power(np.abs(X[idx_left, tree.coord] - tree.b), order) + budget[idx_left] < eps ** order)
        idx_right_increase = (X[idx_right, tree.coord] < tree.b) & (np.power(np.abs(X[idx_right, tree.coord] - tree.b), order) + budget[idx_right] < eps ** order)
        budget_left[idx_left_increase] += np.power(np.abs(X[idx_left, tree.coord] - tree.b), order)[idx_left_increase]
        budget_right[idx_right_increase] += np.power(np.abs(X[idx_right, tree.coord] - tree.b), order)[idx_right_increase]

        # print(budget_left)

        tree.left = self.fit_tree_Lp(X_index[idx_left], X[idx_left], y[idx_left], eps, depth+1, order, budget_left, box_left, next_coord_to_ignore)
        tree.right = self.fit_tree_Lp(X_index[idx_right], X[idx_right], y[idx_right], eps, depth+1, order, budget_right, box_right, next_coord_to_ignore)

        index_left = X_index[idx_left]
        index_right = X_index[idx_right]

        # TODO: here we should update the map
        # print(self.pre_leaf_node_list)
        if tree.left is None:
            leaf_nodes = self.pre_leaf_node_list
            y_min_left = np.zeros(np.sum(idx_left)) + np.inf
            for node in leaf_nodes:
                if node.box.is_intersect(box_left):
                    # box_intersect = node.box.get_intersection(box_left)
                    
                    intersect_index = set(node.y_min_value_map.keys()).intersection(set(index_left))
                    for i, (index, X_i, y_i) in enumerate(zip(index_left, X[idx_left], y[idx_left])):
                        # if point_box_dist(X_i, np.asarray(list(box_intersect.intervals.keys())), np.asarray(list(box_intersect.intervals.values())), order) > eps:
                        #     continue
                        if index not in intersect_index:
                            continue
                        # if index in node.y_min_value_map:
                        y_min_left[i] = min(y_min_left[i], node.y_min_value_map[index] + tree.w_l * y_i)
                    # update y_min_left
            self.leaf_node_list.append(LeafNode(box_left, w_l, dict(zip(index_left, y_min_left))))
        if tree.right is None:
            leaf_nodes = self.pre_leaf_node_list
            y_min_right = np.zeros(np.sum(idx_right)) + np.inf
            for node in leaf_nodes:
                if node.box.is_intersect(box_right):
                    # box_intersect = node.box.get_intersection(box_right)
                    
                    intersect_index = set(node.y_min_value_map.keys()).intersection(set(index_right))
                    for i, (index, X_i, y_i) in enumerate(zip(index_right, X[idx_right], y[idx_right])):
                        # if point_box_dist(X_i, np.asarray(list(box_intersect.intervals.keys())), np.asarray(list(box_intersect.intervals.values())), order) > eps:
                        #     continue
                        # if index in node.y_min_value_map:
                        if index not in intersect_index:
                            continue
                        y_min_right[i] = min(y_min_right[i], node.y_min_value_map[index] + (tree.w_l + tree.w_r) * y_i)
                    # update y_min_left

            self.leaf_node_list.append(LeafNode(box_right, w_l + w_r, dict(zip(index_right, y_min_right))))
    ...
    def fit_tree(self, X, y, gamma, model, eps, depth): # Yihan: TODO: here we should also provide the budget of Xs.
        """
        Recursive procedure for building a single tree.
        ...
        """
        parallel = False  # causes a minor memory leak; disable if the memory is limited
        ...
        # else:
            # raise NotImplementedError("orders other than -1 have not been implemented yet.")

            # w_l, w_r, b, coord, loss = self.fit_stumps_over_coords_Lp(X, y, eps, depth)
        ...
        if parallel and depth <= 4:
            ...
            # if tree.left is None: # here this child is a leaf node
            #     pass
            # if tree.right is None:
            #     pass
    ...
    def fit_stumps_over_coords_Lp(self, X_index, X, y, budget, pre_leaf_nodes, eps, max_eps, order, depth, box, coord_to_ignore = ()):
        # pre_tree = self.trees[-1]
        # leaf_nodes = self.pre_leaf_node_list
        ...
        features_to_check = np.where(idx_non_trivial)[0][:self.n_trials_coord]
        ...
        for i_coord, coord in enumerate(features_to_check):
            if coord in coord_to_ignore:
                continue
            min_losses[i_coord], *params[i_coord, :] = fit_stump_Lp(
                    X_index, X, X[:, coord], y, pre_leaf_nodes, eps, max_eps, budget, coord, order, self.n_bins, self.min_samples_leaf, self.max_weight, box)

        # print(min_losses, params)
        ...
        # if verbose:
        print('[{}-vs-all] depth {}: n_ex {}, n_coords {} -- loss {:.5f}, b={:.3f} wl={:.3f} wr={:.3f} at coord {}'.format(
            self.idx_clsf, depth, X.shape[0], n_coords, min_loss, best_b, best_wl, best_wr, best_coord))
