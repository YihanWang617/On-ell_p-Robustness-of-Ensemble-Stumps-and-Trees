...
for i in range(1, n_trees + 1):
    ...
    begin = time.time()
    weak_learner = ensemble.fit_stumps_over_coords(X_train, y_train, gamma, model, eps, order = -1)
    ...
    threshold_cumu_value = ensemble.build_cumu_threshold_value()
    min_yf_test = ensemble.certify_Lp_bound(X_test, y_test, eps_test, threshold_cumu_value, order = order, precision = 0.0001)
    ...
    # min_L1_robust_yf_test = ensemble.certify_Lp_bound(X_test, y_test, eps, None, (), 1, 0.001)
    # min_yf_test = min_L1_robust_yf_test.min(axis = 1)
...
ensemble.export_json("{}_{}_{}.json".format(dataset, eps, model))
ensemble.save("{}_{}_{}.ensemble".format(dataset, eps, model))
